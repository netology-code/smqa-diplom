...
Файл apk можно скачать тут  ( могут отображаться оповещения от браузера о небезопасном скачивании так как вы устанавливаете не из официального магазина приложения. Сам по себе файл безопасен)
Для работы потребуется android девайс и настроенный сниффер Charles Proxy (вспоминаем лекцию по снифферу) 
Все запросы приложения вы найдете по адресу  https://api.zahleb.me 
Внимательно изучите запросы приложения, как они работают. проверьте заголовки запросов, их ответы.  
Откройте любую понравившуюся книгу и  найдите в ответе  запроса книги параметр , который отвечает за количество просмотров книги

Вам нужно протестировать на клиенте вёрстку и проверить, как будет отображаться большое количество просмотров, если книга будет очень популярной. Доступа к  Базе данных , где вы могли изменить количество, нет .   Зато есть Charles :) 
Необходимо изменить параметр в ответе сервера. Для этого используйте функцию Rewrite  или Break points 

Решением задачи будет предоставленный скриншот настроек используемой функции Charles  и скриншот экрана приложения с измененным количеством просмотров в выбранной книге. 
